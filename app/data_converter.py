...
class DataConverter:
    def __init__(self, download_new_data, countries_to_output, orchestration_mapper, score_manager):
    ...
    def initialise_df(self, download_new_data):
        ...
        df = df.loc[df['country'].isin(self.countries_to_output)]
        ...
        monthly_df = df.groupby(['name', 'year_month'])[
    ...
    def to_score(self):
        for country in self.countries_to_output:
            self.convert_country_data_to_score(country)

    def convert_country_data_to_score(self, country):
        filtered_df = self.df.loc[self.df['name'] == country]
        ...
        for monthly_deaths in monthly_deaths_data:
            if monthly_deaths == 0:
                ...
                self.score_manager.add_bar(country, dynamic, clip)
